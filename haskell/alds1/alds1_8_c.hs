  ...
  where f :: BSTree Int -> ByteString -> IO (BSTree Int)
        f tr bs = case B.words bs of
                   [com, bn] | com == sfind -> do
                                 ...
                                 return tr
                             | com == sdelete -> do
                                 return (bstDelete (readi B.readInt bn) tr)
                             | otherwise -> do
                                 return (bstInsert (readi B.readInt bn) tr)
                   _ -> do
...
bstFindMin :: BSTree a -> Maybe a
bstFindMin Leaf = Nothing
bstFindMin (Node x Leaf _) = Just x
bstFindMin (Node _ lt _) = bstFindMin lt

bstDelete x Leaf = Leaf
bstDelete x (Node y lt rt) | x < y = Node y (bstDelete x lt) rt
                           | x > y = Node y lt (bstDelete x rt)
                           | otherwise = case (lt, rt) of
                                          (Leaf, Leaf) -> Leaf
                                          (Leaf, rt) -> rt
                                          (lt, Leaf) -> lt
                                          _ -> let (Just mn) = bstFindMin rt
                                               in Node mn lt (bstDelete mn rt)
